...
# Возвращает 0 (да) если выбрано "1", 1 (нет) если "2".
ask_yes_no() {
    local prompt="$1"
    local default="${2:-no}"
    local def_label
...
if [[ "$NEW_SSH_PORT" == "$CURRENT_SSH_PORT" ]]; then
    ...
    echo
    if ask_yes_no "Пропустить смену порта и сразу перейти к настройке ufw + fail2ban?" "yes"; then
        SKIP_MIGRATION=true
...
warn "=========================================================="
echo
if ! ask_yes_no "Подключение на новом порту ${NEW_SSH_PORT} точно работает? Продолжить и закрыть старый порт?" "no"; then
